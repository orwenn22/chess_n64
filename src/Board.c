...
extern sprite_t *chess_tileset;
extern joypad_buttons_t pressed_buttons_state;
...
void UpdateBoard(Board* b) {
    ...
    b->overred_x += pressed_buttons_state.d_right - pressed_buttons_state.d_left;
    b->overred_y += pressed_buttons_state.d_down - pressed_buttons_state.d_up;

    if(b->overred_x < 0) b->overred_x = 0;
    else if(b->overred_x > 7) b->overred_x = 7;

    if(b->overred_y < 0) b->overred_y = 0;
    else if(b->overred_y > 7) b->overred_y = 7;


    if(pressed_buttons_state.a) {
        BoardHandleClick(b);
    }
}
...
void DrawBoard(Board* b) {
    ...
    const color_t grid_colors[] = {
            {255, 255, 255, 255},
            {0, 0, 0, 255},
    };

    //C'est plus efficace de faire 2 loops séparés affin de ne pas spammer le rdp de demmandes de changement de couleurs
    rdpq_set_mode_fill(grid_colors[0]);
    for(int y = 0; y < 8; y++) {
        for(int x = 0; x < 8; x++) {
            if((x+y)%2 == 0) rdpq_fill_rectangle(x * w, y * h, x*w+w, y*h+h);
        }
    }
    rdpq_set_mode_fill(grid_colors[1]);
    for(int y = 0; y < 8; y++) {
        for(int x = 0; x < 8; x++) {
            if((x+y)%2 == 1) rdpq_fill_rectangle(x * w, y * h, x*w+w, y*h+h);
        }
    }
    ...
    for(int y = 0; y < 8; y++) {
        for(int x = 0; x < 8; x++) {
            ...
            if((pawn & 0x0f) != PawnType_None) {
                ...
                //Texture de la pièce
                rdp_load_texture_stride(0, 0, MIRROR_DISABLED, chess_tileset, 3 + team*6 + texture_id);
                rdp_draw_sprite_scaled(0, x * w, y * h, (float)w/16.f, (float)h/16.f, MIRROR_DISABLED);
            }

            //Si le move de la pièce selectionné est possible sur cette case l'afficher en vert
            if(b->possible_moves[x + y*8] == 1) {
                rdp_load_texture_stride(0, 0, MIRROR_DISABLED, chess_tileset, 2);
                rdp_draw_sprite_scaled(0, x * w, y * h, (float)w/16.f, (float)h/16.f, MIRROR_DISABLED);
            }
        }
    }
    ...
    if(b->selected_cell != -1) {
        int x = b->selected_cell%8 * w;
        int y = b->selected_cell/8 * h;
        rdp_load_texture_stride(0, 0, MIRROR_DISABLED, chess_tileset, 1);
        rdp_draw_sprite_scaled(0, x, y, (float)w/16.f, (float)h/16.f, MIRROR_DISABLED);
    }

    //Case survollé
    rdp_load_texture_stride(0, 0, MIRROR_DISABLED, chess_tileset, 15);
    rdp_draw_sprite_scaled(0, b->overred_x*w, b->overred_y*h, (float)w/16.f, (float)h/16.f, MIRROR_DISABLED);
}
...
static void BoardHandleClick(Board* b) {
    ...
    if((cell >> 4) != b->current_turn) {
        ...
        if(b->possible_moves[overred_index] == 1) {
            ...
            b->cells[overred_index] = b->cells[(int)b->selected_cell];
            b->cells[(int)b->selected_cell] = 0;
            ...
            if(CheckCanCaptureKing(b, other_team)) {
                printf("OH NO\n");
                //Undo le mouvement
                b->cells[(int)b->selected_cell] = b->cells[overred_index];
